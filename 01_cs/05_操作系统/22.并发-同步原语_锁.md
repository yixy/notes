...
处理器提供**总线锁定机制**，当一个处理器要操作共享变量时，在 BUS 总线上发出一个 Lock 信号，其他处理就无法操作这个共享变量了。
...
* 现代的处理器基本都支持和使用的缓存锁定机制。早期有些处理器不支持缓存锁定。对于Intel486和Pentium处理器，就算锁定的内存区域在处理器的缓存行中也会调用总线锁定。

> 后来的处理器都提供了缓存锁定机制，也就说当某个处理器对缓存中的共享变量进行了操作，其他处理器会有个嗅探机制，将其他处理器的该共享变量的缓存失效，待其他线程读取时会重新从主内存中读取最新的数据，基于 MESI 缓存一致性协议来实现的。

###### 2.4 指令集中的硬件同步语义：atomi exchange & compare-and-exchange & 链接的加载和条件式存储指令 &fetch-and-add

**CPU通常提供了一些处理器指令来保障原子性操作。**这些指令如下所示。其中前面两条大部分处理器已经实现，后面的两条是现代处理器当中新增加的。而且根据不同的体系结构，指令存在着明显差异。

* 测试并设置（Tetst-and-Set/atomi exchange）
* 获取并增加（Fetch-and-Increment/fetch-and-add）
* 比较并交换（Compare-and-Swap/compare-and-exchange）
* 加载链接/条件存储（Load-Linked/Store-Conditional）

**在硬件层面上，这些指令一般都是通过锁总线或者锁cache line的方式实现的。**在x86体系里，就是LOCK前缀指令，以及所有隐含LOCK的指令（如XCHG等）。从公开的资料上看，486之前，LOCK是锁总线的，486以后，大概就是锁cache line+MESI协议。而操作系统提供的原子操作（比如CAS），大部分都用到的LOCK前缀指令。
...
大多数处理器中的atomi exchange、 compare-and-exchange/CAS 、链接的加载和条件式存储指令 、fetch-and-add 等处理器原子指令可能采用缓存锁定实现，受限更少，性能更高，但是只依赖这些指令原语是没办法在代码中实现对临界区加锁。如果要实现锁（比如中操作系统中），可能还需要一些简单的机制，下面分别介绍自旋锁、阻塞唤醒队列模式、两阶段锁模式。
